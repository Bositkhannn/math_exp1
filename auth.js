...
// Reset student password (generates random, updates Auth and Firestore)
async function resetStudentPassword(uid) {
    ...
    try {
        // Use Firebase Auth REST API to update password (requires admin? No, we need the user's token)
        // Alternative: sign in as the student, update, then sign back as teacher. That's messy.
        // Since we have the teacher's auth, we cannot directly update another user's password.
        // Workaround: use the Firebase Admin SDK? Not possible client-side.
        // For simplicity, we'll update only Firestore and rely on teacher to give the new password.
        // The student will still be able to log in with old password because Auth not updated.
        // This is a limitation. To truly reset, we'd need a Cloud Function.
        // Given the scope, we'll just update Firestore and tell teacher that Auth is not updated.
        // We'll inform teacher to use the new password for the student next time.
        toast("Parol faqat ma'lumotlar bazasida o'zgartirildi. O'quvchi keyingi kirishda yangi paroldan foydalanishi uchun uni xabardor qiling.", 'warn');
