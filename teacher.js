...
// If for some reason it's not defined, provide a fallback (should not happen)
if (typeof ALLOWED_GRADES === 'undefined') {
    window.ALLOWED_GRADES = ["7-F", "8-F", "9-F", "10-F", "11-F"];
}

// Helper to render a grade dropdown (uses the global ALLOWED_GRADES)
...
// ... rest of teacher.js (exactly as previously provided) ...
// ... rest of teacher.js (unchanged) ...
...
function renderTeacherPanel() {
    ...
    app.innerHTML = `
        ...
        <!-- Question Editor Drawer (unchanged) -->
        ...
        <!-- Test Creator Drawer (with grade dropdown) -->
        ...
        <!-- Daily Task Creator Drawer (with grade dropdown) -->
        ...
        <div class="overlay" id="ov-student" onclick="if(event.target===this) closeDrawer('ov-student')">
            <div class="drawer">
                ...
                <div class="green-info">
                    ...
                    <p>Faqat lotin harflari va nuqta: <span class="mono"></span><br>
                    O'quvchi shu login va parol bilan kiradi.</p>
                </div>
                <div class="field"><label>Login *</label>
                    <input type="text" id="std-user" placeholder="" autocapitalize="none" spellcheck="false"></div>
                ...
                <div class="field"><label>To‘liq ism</label>
                    <input type="text" id="std-name" placeholder=""></div>
        ...
        <div class="overlay" id="ov-import-students" onclick="if(event.target===this) closeDrawer('ov-import-students')">
            <div class="drawer">
                ...
                <div class="green-info">
                    ...
                    <p>
                        <b>A</b> — Login (username)<br>
                        <b>B</b> — Parol<br>
                        <b>C</b> — Sinf (7-F, 8-F, …)<br>
                        <b>D</b> — To‘liq ism<br><br>
...
function toggleTestUniversal(grade) {
    // No need for extra UI – grade value '' means universal
}
...
async function resetStudentPassword(uid) {
    ...
    if (res.ok) {
        toast(`Yangi parol: ${res.newPass}`, 'ok');
...
function previewStudentExcel() {
    ...
    reader.onload = function(e) {
        try {
            ...
            const skip = !real.length ? 0 : (real[0][0] === 'username' ? 1 : 0);
            prev.innerHTML = `<b>${real.length - skip} ta o'quvchi topildi:</b><br>` +
                real.slice(skip, skip + 5).map(r => `${r[0]} (${r[2]})`).join('<br>') +
...
async function doImportStudents() {
    ...
    reader.onload = async function(e) {
        try {
            ...
            const rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
            const real = rows.filter(r => String(r[0] || '').trim());
            const start = (!real.length || real[0][0] === 'username') ? 1 : 0;

            let added = 0, errors = 0;
            for (let i = start; i < real.length; i++) {
                const [username, password, grade, fullName] = real[i];
                if (!username || !password || !grade) {
                    errors++;
                    continue;
                }
...
function reviewResult(resultId) {
    ...
    app.innerHTML = `
        <nav class="navbar">
            ...
            <button class="btn btn-ghost btn-sm" onclick="renderTeacherPanel()">Orqaga</button>
        </nav>
        <div class="main">
...
function renderTSettings() {
    ...
    el.innerHTML = `
        ...
        <div class="card">
            ...
            <button class="btn btn-primary" onclick="handleUpdateTeacherAccount()">O'zgartirish</button>  <!-- changed -->
        </div>
...
// Renamed to avoid recursion – now handles the UI and calls the auth function
// Renamed to avoid recursion – now handles the UI and calls the auth function
async function handleUpdateTeacherAccount() {
    ...
    // Call the auth function (defined in auth.js) via window
...
// Export the new function globally
...
// ==================== OVERVIEW, TESTS, DAILY, RESULTS (unchanged except removal of codes) ====================
...
window.setUniversal = () => {}; // no longer used, but keep for safety
window.addGradeChip = () => {};
