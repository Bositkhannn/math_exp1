            ...
            return `<div class="card">
                <div class="card-row">
                    ...
                    <div style="text-align:right;flex-shrink:0">
                        ...
                        <br>
                        <button class="btn btn-ghost btn-sm" style="margin-top:.4rem" onclick="reviewMyResult('${r.id}')">👁 Ko'rib chiqish</button>
                    </div>
                </div>
            </div>`;
        }).join('');
...
async function submitTest() {
    ...
    const res = {
        ...
        type: obj.type,
        testId: obj.id,
        qids: obj.qids,          // ✅ save question ids
        answers: testAnswers,    // ✅ save answers
        score,
        total: qs.length,
...
function showResult(res) {
    ...
    app.innerHTML = `
        <div class="result-box" style="margin: 2rem auto; padding: 1rem;">
            ...
            <button class="btn btn-primary btn-full btn-lg" onclick="reviewMyResult('${res.id}')">👁 Ko'rib chiqish</button>
            <button class="btn btn-ghost btn-full btn-lg" style="margin-top:.5rem" onclick="navigateTo('student')">← Bosh sahifaga</button>
        </div>
    `;
}
...
// ==================== STUDENT REVIEW ====================
function reviewMyResult(resultId) {
    const result = DB.results.find(r => r.id === resultId);
    if (!result) { toast("Natija topilmadi", 'err'); return; }

    const qids = result.qids || (() => {
        const t = DB.tests.find(t => t.id === result.testId);
        const d = DB.dailyTasks.find(d => d.id === result.testId);
        return (t || d)?.qids || [];
    })();
    ...
    app.innerHTML = `
        <nav class="navbar">
            <div class="navbar-brand" style="font-size:.95rem">👁 Ko'rib chiqish</div>
            <button class="btn btn-ghost btn-sm" onclick="navigateTo('student')">← Orqaga</button>
        </nav>
        <div class="main">
            <div class="card" style="text-align:center;margin-bottom:1rem">
                <div style="font-size:1.1rem;font-weight:900;margin-bottom:.25rem">${esc(result.title)}</div>
                <div class="muted" style="font-size:.82rem;margin-bottom:.5rem">${result.date}</div>
                <span class="badge ${gradeBadge(result.score, result.total)}" style="font-size:.9rem;padding:.3rem .9rem">
            ...
            ${qs.map((q, i) => {
                ...
                return `
                <div class="q-block" style="border-color:${isCorrect ? 'var(--green)' : userAns !== undefined ? 'var(--red)' : 'var(--border)'}">
                    <div class="q-num">
                        SAVOL ${i + 1}
                        ${isCorrect ? '<span style="color:var(--green);margin-left:.5rem">✓ To\'g\'ri</span>' 
                          : userAns !== undefined ? '<span style="color:var(--red);margin-left:.5rem">✗ Noto\'g\'ri</span>'
                          : '<span style="color:var(--muted);margin-left:.5rem">— Javob berilmadi</span>'}
                    </div>
                    <div class="q-text">${esc(q.text)}</div>
                    ${q.img ? `<img class="q-img" src="${q.img}" alt="">` : ''}
                    <div class="options-grid">
                        ${q.opts.map((o, j) => {
                            ...
                            else if (j === userAns && j !== q.correct) cls = 'wrong';
                            return `<div class="opt-btn ${cls}" style="cursor:default">
                                <span class="opt-letter">${'ABCD'[j]}</span>
                                <span style="flex:1">${esc(o)}</span>
                                ${j === q.correct ? '<span style="color:var(--green);font-weight:900;margin-left:auto">✓</span>' : ''}
                                ${j === userAns && j !== q.correct ? '<span style="color:var(--red);font-weight:900;margin-left:auto">✗</span>' : ''}
                            </div>`;
                        }).join('')}
                    </div>
                </div>`;
...
window.reviewMyResult = reviewMyResult;   // ✅ export review function